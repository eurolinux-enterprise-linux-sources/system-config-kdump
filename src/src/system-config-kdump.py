...
class MainWindow:
    ...
    def __init__(self):
        ...
        if not os.access(main_icon, os.F_OK):
            main_icon = "/usr/share/icons/hicolor/48x48/apps/system-config-kdump.png"
        self.toplevel.set_icon_from_file(main_icon)
        ...
        self.basic_settings          = self.xml.get_widget("SWBasicSettings")
...
#        self.default_initrd_radio_button = self.xml.get_widget("defaultInitrdRadiobutton")
	...
	# read current kdump settings from grubby
        ...
        self.basic_settings.show_all()
        if total_mem < auto_thresh:
